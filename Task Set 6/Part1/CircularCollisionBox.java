...
public class CircularCollisionBox
{
	...
	public boolean collidesWith(SquareCollisionBox box)
	{
		...
		if (distanceX > (box.getSideLength()/2 + r)) { return false; }
		if (distanceY > (box.getSideLength()/2 + r)) { return false; }

		if (distanceX <= (box.getSideLength()/2)) { return true; } 
		if (distanceY <= (box.getSideLength()/2)) { return true; }

		double cornerDistanceSq = (distanceX - box.getSideLength()/2)*(distanceX - box.getSideLength()/2) +
							 (distanceY - box.getSideLength()/2)*(distanceY - box.getSideLength()/2);
